# app/models.py (VERSION FINALE POUR RÉINITIALISATION MIGRATIONS)
...
from app import db, login # S'assure que db et login sont initialisés dans app/__init__.py
...
class Task(db.Model):
    ...
    def get_target_countries_list(self):
        return ['ALL'] if not self.target_countries or self.target_countries.upper() == 'ALL' else [c.strip().upper() for c in self.target_countries.split(',')]

    def get_target_devices_list(self):
        return ['ALL'] if not self.target_devices or self.target_devices.upper() == 'ALL' else [d.strip().capitalize() for d in self.target_devices.split(',')]
...
class UserTaskCompletion(db.Model):
    ...
    def __repr__(self):
        return f'<User {self.user_id} completed Task {self.task_id}>'
...
class ExternalTaskCompletion(db.Model):
    ...
    def __repr__(self):
        return f'<External Completion Task {self.task_id} via Referrer {self.referrer_user_id} - Status: {self.status}>'
...
class ReferralCommission(db.Model):
    ...
    def __repr__(self):
        return f'<ReferralCommission {self.id} - Referrer {self.referrer_id} from User {self.referred_user_id} - Amount {self.commission_amount} - Status {self.status}>'
...
class Post(db.Model):
    ...
    def generate_unique_slug(title, post_id=None):
        ...
        while True:
            ...
            # Si le slug existe, ajoute un suffixe aléatoire court
