# app/models.py (VERSION FINALE COMPLÈTE - AVEC CustomScript.description)
...
class User(UserMixin, db.Model):
    ...
    is_admin = db.Column(db.Boolean, default=False, nullable=False) # Rendu nullable=False
...
class Task(db.Model):
    ...
    is_active = db.Column(db.Boolean, default=True, nullable=False) # Rendu nullable=False
...
class ExternalTaskCompletion(db.Model):
    ...
    status = db.Column(db.String(20), default='Pending', nullable=False) # Rendu nullable=False
...
class Notification(db.Model):
    ...
    is_read = db.Column(db.Boolean, default=False, nullable=False) # Rendu nullable=False
...
class CustomScript(db.Model):
    ...
    location = db.Column(db.String(length=10), nullable=False, default='head')
    excluded_endpoints = db.Column(db.Text, nullable=True)
    description = db.Column(db.Text, nullable=True) # <<< CHAMP DESCRIPTION EST BIEN LÀ
    is_active = db.Column(db.Boolean, default=False, nullable=False, index=True)
